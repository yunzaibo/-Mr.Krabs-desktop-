...
| **Image/Video Generation** | Zhipu CogView-4 image generation + CogVideoX-2 video generation, unified text dialog entry, generated results saved as URL references, inline preview + download button |
...
| **Autonomous Agents** | 3D budget guardrails (token/time/cost), code execution sandbox, Checkpoint long-task recovery |
...
| **Skill System** | Skill marketplace + custom skills + LLM-powered Skill creation, Skill Chain invocation, dependency management, Tool registration & per-tool permissions |
| **MCP Protocol** | Model Context Protocol tool integration (stdio/SSE/Streamable HTTP), OAuth 2.0+PKCE authentication, command allowlist security validation |
| **Workflow Canvas** | Visual drag-and-drop Agent workflow orchestration, DAG execution engine |
| **Knowledge Base (RAG)** | Document upload/parsing/vector retrieval, supports PDF / Markdown / TXT formats; Auto-RAG auto-retrieves knowledge base and injects context |
...
| **Security Gateway** | Prompt injection detection / tool output sanitization / PII filtering / content filtering / RBAC access control / SSRF protection |
...
| **Deep Research** | 4-stage autonomous research (search → analysis → synthesis → report), based on Plan-and-Execute engine |
...
| **Global Shortcut** | `Cmd+Shift+H` to summon Quick Chat window anytime |
...
> If you've already downloaded manually, run `xattr -cr /Applications/HexClaw.app` in Terminal to bypass the block.
...
# 3. Build Go sidecar (first time only)
make sidecar
...
│   │   ├── im-channels.ts            # IM channel API
│   │   ├── team.ts                   # Team collaboration API
│   │   ├── voice.ts                  # Voice API (TTS/STT)
│   │   ├── voicechat.ts              # Voice chat API
...
│   │   ├── models.ts                 # Model list API
│   │   ├── ollama.ts                 # Ollama local model API
│   │   ├── imagegen.ts               # Image generation API
│   │   ├── videogen.ts               # Video generation API
│   │   ├── capabilities.ts           # Capabilities API
│   │   └── tools-status.ts           # Tool status API
│   ├── components/                   # Components
│   │   ├── layout/                   # Layout (AppLayout/Sidebar/TitleBar/ContextBar)
│   │   ├── chat/                     # Chat (ChatInput/SessionList/MarkdownRenderer/ToolApprovalCard etc.)
│   │   ├── agent/                    # Agent (AgentCard/AgentForm/AgentStatus)
│   │   ├── artifacts/                # Artifacts (ArtifactsPanel/ArtifactPreview/ArtifactCodeView)
│   │   ├── inspector/                # Right-side details (InspectorContext/ContextCard/KeyValueRow)
│   │   ├── canvas/                   # Canvas (TemplateGallery)
│   │   ├── automation/               # Automation
│   │   ├── workspace/                # Workspace (TaskListPanel)
│   │   ├── runtime/                  # Runtime (TaskMonitorPanel/TimelinePanel)
│   │   ├── settings/                 # Settings (SettingsNotification/SettingsSecurity)
│   │   ├── logs/                     # Logs (LogEntry/LogFilter/LogStats)
│   │   └── common/                   # Common (CommandPalette/ConfirmDialog/Toast/ErrorBoundary)
│   ├── views/                        # Page views
...
│   │   ├── ChannelsView.vue          # Channel management
│   │   ├── TeamView.vue              # Team collaboration
│   │   ├── RuntimeView.vue           # Runtime management
...
│   ├── stores/                       # Pinia state management (thin stores)
...
│   │   └── settings.ts               # Settings (LLM + security + notifications)
...
│   │   ├── useShortcuts.ts           # In-app shortcuts (Cmd1~7 page switching)
...
│   │   ├── useVoice.ts               # Voice features (TTS/STT)
│   │   ├── useToast.ts               # Toast notifications
│   │   ├── useWorkspace.ts           # Workspace management
│   │   ├── useAssetRuntime.ts        # Asset runtime management
│   │   ├── useContextRuntime.ts      # Context runtime
│   │   ├── usePersistenceRuntime.ts  # Persistence runtime
│   │   └── useRecoveryRuntime.ts     # Recovery runtime
│   ├── services/                     # Business logic service layer
│   │   ├── chatService.ts            # Chat service (WebSocket/HTTP send)
│   │   ├── messageService.ts         # Message service (message construction/persistence)
│   │   ├── skillBridge.ts            # Skill invocation bridge
│   │   ├── skillLoader.ts            # Skill loader
│   │   ├── skillRegistry.ts          # Skill registry
│   │   ├── skillExecutor.ts          # Skill script executor
│   │   ├── agentExecutor.ts          # Agent executor
│   │   ├── agentRegistry.ts          # Agent registry
│   │   ├── taskExecutor.ts           # Task executor
│   │   ├── runtimeBridge.ts          # Runtime bridge
│   │   ├── runtime/                  # Runtime sub-modules
│   │   ├── contextLoader.ts          # Context layer loader
│   │   ├── contextManager.ts         # Context lifecycle manager
│   │   ├── commandRegistry.ts        # Command registry
│   │   ├── hookRegistry.ts           # Hook registry
│   │   ├── hookExecutor.ts           # Hook executor
│   │   ├── intentMatcher.ts          # Intent matcher (NL trigger)
│   │   ├── workspaceProjector.ts     # Workspace projector
│   │   ├── claudeCodeImporter.ts     # Claude Code format importer
│   │   ├── providerAdapter.ts        # LLM provider adapter
│   │   ├── agentAdapter.ts           # Agent adapter
│   │   ├── backendLLMClient.ts       # Backend LLM client
│   │   ├── capabilityRegistry.ts     # Capability registry
│   │   └── capabilityValidator.ts    # Capability validator
...
│   │   ├── navigation.ts             # Navigation registry (3-tier grouping)
│   │   ├── providers.ts              # LLM Provider configuration
│   │   └── skills-marketplace.ts     # Skills marketplace config
...
│   │   └── skill_sandbox.rs          # Skill sandbox
│   ├── binaries/                     # Go sidecar binaries (build output)
│   ├── skills/                       # Built-in Skill resources
│   ├── icons/                        # App icons
...
│   └── claude-code-practices/        # Claude Code Development SOP
...
Sidecar binaries are output to the `src-tauri/binaries/` directory and automatically embedded by Tauri during packaging.
...
3. If curl succeeds but the frontend still shows stopped, re-run `make build` and install the latest version

### `make sidecar` build fails

1. Verify Go >= 1.25 is installed: `go version`
2. Verify GitHub access and remote source fetch
3. Verify Rust toolchain is installed: `rustc -vV`
